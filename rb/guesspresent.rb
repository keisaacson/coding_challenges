...
# Possible values for size: "small", "medium", "large" Possible values for clatters: "no", "a bit", "yes" Possible values for weight: "light", "medium", "heavy" The return value must be an array of the names of items from your wishlist, e.g. ["Toy Car", "Card Game"] Don't add any item more than once to the result The order of names in the returned array doesn't matter It's possible, that multiple items from your wish list have the same attribute values. If they match the attributes of one of the presents, add all of them.
...
def guess_gifts(wishlist, presents)
  wishlistNames = wishlist.map {|x| x.delete(:name)}

  matches = wishlist & presents

  indexArray = wishlist.each_index.select {|i| matches.include?(wishlist[i])}

  indexArray.map {|x| wishlistNames[x]}
